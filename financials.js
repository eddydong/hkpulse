// --- Stock Data Utility Functions (moved from stock-data.js) ---
...
async function fetchTickersMeta() {
	try {
		...
		const url = `https://get-stock-tickers-v436pnaqfa-df.a.run.app/?market=${encodeURIComponent(market)}`;
...
async function fetchAndRenderForCompany(symbol) {
	...
	try {
		setDefaultDates();
		const market = localStorage.getItem('selectedMarket') || 'hk';
		const url = buildDataUrl(symbol) + `&market=${encodeURIComponent(market)}`;
